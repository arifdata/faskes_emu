...
def init_data_pasien():
    with open('utils/data_pasien.csv', 'r') as f:
        rd = csv.reader(f)
        for i in rd:
            a = DataPasien(no_kartu=i[1], nama_pasien=i[0], alamat=i[3], usia=datetime.datetime.strptime(i[2], '%d-%m-%Y'), no_hp=00000)
            a.save()
            print("Menginput {} {} {} {}".format(i[0], i[1], i[2], i[3]))

def init_data_obat():
    with open('utils/daftar_obat.csv', 'r') as f:
        rd = csv.reader(f)
        for i in rd:
